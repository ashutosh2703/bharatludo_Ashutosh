...
const Withdrawopt = ({ walletUpdate }) => {
  ...
  const [isManualPayoutActive, setManualPayoutActive] = useState(true);
  ...
  const [autoPayoutBy, setAutoPayoutBy] = useState(false);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setManualPayoutActive(true);

        setRazorPayPayoutAuto(false);
...
//console.log(autoPayout);
  ...
  const payoutFromRazorpay = () => {
    console.log(" my payzone");
    if (amount && amount >= 100 && amount <= 50000 && type) {////////////// addd 100 here 
      // e.preventDefault();
      const payment_gatway = "mypay";
      const access_token = localStorage.getItem("token");
      const headers = {
        Authorization: `Bearer ${access_token}`,
      };
      ...
      } else if (100 <= amount <= 50000) {///////////////////addd 100 here
        msg = "amount should be more than 100 and less then 50000.";
      }
      Swal.fire({
        title: msg,
  ...
  const payoutFromRazorpayAdmin = () => {
    console.log(" my payzone above 10");
//    if (amount && amount >= 2000 && amount <= 50000 && type) {
      // e.preventDefault();
      const payment_gatway = "mypay";
      const access_token = localStorage.getItem("token");
      const headers = {
